import { assertListType, GraphQLInt, GraphQLList, GraphQLNonNull, GraphQLString } from 'graphql';
...
import {
    ...
    ConstBoolQueryNode,
    FLEX_SEARCH_CURSOR_ON_UNINDEXED_SORT_ERROR,
        ...
        return {
            ...
            resolve: (sourceNode, args, info) => {
                ...
                if (listNode instanceof FlexSearchQueryNode) {
                    ...
                    // if we need an absolute ordering, see if the primary sort is fitting because that would be efficient
                    if (
                        isAbsoluteOrderRequired &&
                        orderArgMatchesPrimarySort(args[ORDER_BY_ARG], listNode.rootEntityType.flexSearchPrimarySort)
                    ) {
                    ...
                    if (isCursorRequested || !!afterArg) {
                        ...
                        if (violatingClauses.length) {
                            return new RuntimeErrorQueryNode(
                                ...
                                {
                                    code: FLEX_SEARCH_CURSOR_ON_UNINDEXED_SORT_ERROR
                                }
                            );
                        }
                    }
